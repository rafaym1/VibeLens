...
class TestListSubagents:
    def test_returns_empty_list(self, client):
        res = client.get("/api/extensions/subagents")
    ...
    def test_search_filters(self, client, subagent_service):
        ...
        res = client.get("/api/extensions/subagents?search=alpha")
        data = res.json()
        assert data["total"] == 1
    ...
    def test_pagination(self, client, subagent_service):
        ...
        res = client.get("/api/extensions/subagents?page=2&page_size=2")
        data = res.json()
        assert data["total"] == 3
...
class TestGetSubagent:
    def test_returns_subagent_with_content(self, client, subagent_service):
        subagent_service.install(name="my-subagent", content=SAMPLE_SUBAGENT_MD)
        res = client.get("/api/extensions/subagents/my-subagent")
        assert res.status_code == 200
        data = res.json()
    ...
    def test_not_found(self, client):
        res = client.get("/api/extensions/subagents/nonexistent")
...
class TestModifySubagent:
    def test_updates_subagent(self, client, subagent_service):
        subagent_service.install(name="my-subagent", content=SAMPLE_SUBAGENT_MD)
        res = client.put(
            "/api/extensions/subagents/my-subagent",
            json={"content": "---\ndescription: Updated\n---\nNew body."},
        )
        assert res.status_code == 200
    ...
    def test_not_found(self, client):
        res = client.put(
            "/api/extensions/subagents/nonexistent",
...
class TestUninstallSubagent:
    def test_deletes_subagent(self, client, subagent_service):
        subagent_service.install(name="my-subagent", content=SAMPLE_SUBAGENT_MD)
        res = client.delete("/api/extensions/subagents/my-subagent")
        assert res.status_code == 200
        data = res.json()
        assert data["deleted"] == "my-subagent"

    def test_not_found(self, client):
        res = client.delete("/api/extensions/subagents/nonexistent")
        assert res.status_code == 404
...
class TestSyncSubagent:
    def test_syncs_to_agent(self, client, subagent_service):
        subagent_service.install(name="my-subagent", content=SAMPLE_SUBAGENT_MD)
        res = client.post(
            "/api/extensions/subagents/my-subagent/agents",
            json={"agents": ["claude"]},
        )
        assert res.status_code == 200
        data = res.json()
    ...
    def test_not_found(self, client):
        res = client.post(
            "/api/extensions/subagents/nonexistent/agents",
            json={"agents": ["claude"]},
        )
        assert res.status_code == 404
...
class TestUnsyncSubagent:
    def test_unsyncs_from_agent(self, client, subagent_service):
        ...
        res = client.delete("/api/extensions/subagents/my-subagent/agents/claude")
        assert res.status_code == 200

    def test_agent_not_found(self, client, subagent_service):
        subagent_service.install(name="my-subagent", content=SAMPLE_SUBAGENT_MD)
        res = client.delete("/api/extensions/subagents/my-subagent/agents/unknown")
        assert res.status_code == 404
...
class TestImportFromAgent:
    def test_imports_all(self, client, subagent_service):
        ...
        res = client.post("/api/extensions/subagents/import/claude")
        assert res.status_code == 200
        data = res.json()
        assert "agent-sub" in data["imported"]

    def test_unknown_agent(self, client):
        res = client.post("/api/extensions/subagents/import/unknown")
        assert res.status_code == 404
